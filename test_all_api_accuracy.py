...
df_easre=pd.read_excel('7_easre_train_cleaned')
...
MODEL_easre.load_weights('distillbert_7easre_Model.h5')
...
for l in tqdm(range(len(test_data))):
  for j in ['data','easre','ibgpdts','riskgov','iris','hiri','lcs']:
    try:
      ...
      pass
out=out.pivot_table(index=['utterance','intent','intent1','intent_predicted','Top confidence scores'],columns='domain',values='confidence').reset_index().sort_values('utterance')

out.to_excel('All_models_test_output.xlsx',index=None)
