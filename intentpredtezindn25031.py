...
import torch.nn as nn
import pandas as pd
MODEL = BERTBaseUncased(4095,494)
for i in range(0,3):
    MODEL.load_state_dict(torch.load("/nlv/ForThiya/intentprocess/IRIS_new_model_fold00"+str(i)+".bin"))
    MODEL.to(DEVICE)
    MODEL.eval()
    ...
    sent_csv=pd.read_excel("New_Domain_Data.xlsx",sheet_name="Test",usecols=['utterance', 'intent1'])
    sentence=sent_csv.utterance.values
    # sentence=[sentence[0]]

    # data=pd.read_excel("RNN - Data1.xlsx",sheet_name="Sheet1")
    dmap=pd.read_excel("New_Domain_Data.xlsx",sheet_name="mapping")
