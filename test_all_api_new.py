...
for l in tqdm(range(len(test_data))):
  ...
  for j in ['data','easre','ibgpdts','riskgov','iris','hiri','lcs']: 
    try: 
      ...
      k.drop('domain',axis=1,inplace=True)
      out1=pd.concat([out1,k])
    except:
      pass
  out=pd.concat([out,out1[out1['confidence']==out1['confidence'].max()].drop_duplicates()])
  print("completed prediction for {}. Saving the file..".format(j))
  out[['utterance','intent','intent_predicted','confidence','compare']].to_csv('All_models_test_output.csv',index=None)